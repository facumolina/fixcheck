...
target_test_src = os.path.join(subject_base_dir, tests_src_dir, target_test.replace('.', '/')+'.java')
target_class = subject_data['target_class'].values[0]
input_class = subject_data['input_class'].values[0]

# Run FixCheck
subprocess.run(f'./fixcheck.sh {subject_cp} {test_classes_path} {target_test} {target_test_src} {target_class} {input_class}', shell=True)
