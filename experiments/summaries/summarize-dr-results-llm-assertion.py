...
no_report = []
patches_with_score_greater_than = []
score_threshold = 0.80
for subject_id in os.listdir(results_dir):
    if subject_id.endswith('.csv'):
        continue
    base_folder = os.path.join(subject_id,"llm-assertion")
    ...
    if max_score >= score_threshold:
        patches_with_score_greater_than.append(subject_id)
...
correct_patches_reports.to_csv(results_dir+'/correct_patches_reports-llm-assertion.csv', index=False)
incorrect_patches_reports.to_csv(results_dir+'/incorrect_patches_reports-llm-assertion.csv', index=False)
...
print(f'Patches with score > {score_threshold}: {patches_with_score_greater_than}')
print()
